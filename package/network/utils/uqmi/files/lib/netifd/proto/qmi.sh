...
proto_qmi_setup() {
	...
	dataformat="$(uqmi -s -d "$device" -t 3000 --wda-get-data-format)"
	...
	while true; do
		registration_state=$(uqmi -s -d "$device" -t 3000 --get-serving-system 2>/dev/null | jsonfilter -e "@.registration" 2>/dev/null)
